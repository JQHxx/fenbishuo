...
- (void)makeToast:(NSString *)message {
    if(kIsEmptyString(message)) return;
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position backgroundAlpha:(CGFloat)alpha {
    UIView *toast = [self viewForMessage:message title:nil image:nil backgroundAlpha:alpha];
    [self showToast:toast duration:duration position:position];
}
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    ...
    // default to bottom
    return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalPadding-20);
}
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image backgroundAlpha:(CGFloat)alpha {
    // sanity
    if((message == nil) && (title == nil) && (image == nil)) return nil;
    ...
    wrapperView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:alpha];
