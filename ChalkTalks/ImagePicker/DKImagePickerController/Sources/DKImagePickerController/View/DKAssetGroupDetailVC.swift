...
//        debugPrint("minLocationY:\(minLocationY) maxLocationY:\(maxLocationY) current:\(location.y)")
    ...
    func showGallery(from indexPath: IndexPath) {
        ...
        if imagePickerController.canSelect(asset: asset, showAlert: false) || imagePickerController.contains(asset: asset) {
            showGallery(from: cell)
        } else {
            imagePickerController.canSelect(asset: asset, showAlert: true)
        }
    }
    ...
    func dequeueReusableCameraCell(for indexPath: IndexPath) -> DKAssetGroupDetailBaseCell {
        ...
        cell.imagePickerController = imagePickerController
        return cell
    }
    ...
    func setup(assetCell cell: DKAssetGroupDetailBaseCell, for indexPath: IndexPath, with asset: DKAsset) {
        ...
        guard let vc = imagePickerController else { return }
        if vc.canSelect(asset: asset, showAlert: false) || vc.contains(asset: asset) {
            if cell.showMaskView { cell.showMaskView = false }
        } else {
            cell.showMaskView = true
        }
