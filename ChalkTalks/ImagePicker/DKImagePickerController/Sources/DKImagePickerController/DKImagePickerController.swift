...
open class DKImagePickerController: DKUINavigationController, DKImageBaseManagerObserver, UIAdaptivePresentationControllerDelegate {
    ...
    /// 选取视频
    @objc public var didSelectVideo: ((_ fileUrl: URL) -> Void)?
    ...
    @objc public private(set) lazy var groupDataManager: DKImageGroupDataManager = {
        ...
        let manager = DKImageGroupDataManager(configuration: configuration)
        manager.onlyVideos = self.assetType == .allVideos
        return manager
    }()
...
//        navigationBar.isTranslucent = false
    ...
    func finish(completion: (() -> Void)?) {
        guard
            ...
            else { complete(with: completion); return }
    ...
    func complete(with completion: (() -> Void)?) {
        if needShowPhotoWithAudioEdit {
            if assets.isEmpty {
                didSelectAssets?([])
                dismiss()
            } else {
                showPhotoWithAudioEditor()
            }
            completion?()
            return
        }
        ...
        let completeBlock: ([DKAsset]) -> Void = { [weak self] assets in
            guard let self = self else { return }
            self.exportStatus = .none
        ...
        let exportBlock = { [weak self] in
            ...
            if let exporter = self.exporter {
                self.exportRequestID = exporter.exportAssetsAsynchronously(assets: assets) { [weak self] info in
                    ...
                    if self.exportRequestID == requestID {
                        self.exportRequestID = DKImageAssetExportInvalidRequestID
    ...
    private func showCamera() {
        ...
        let didFinishCapturingImage = { [weak self] (image: UIImage, metadata: [AnyHashable : Any]?) in
            guard let self = self else { return }
            self.saveImage(image, metadata) { [weak self] (asset) in
                if self?.needShowPhotoWithAudioEdit ?? false {
                    self?.dismissCamera()
                }
...
//                        "image" : image,
//                        "didFinishEditing" : didFinishEditing,
//                        "isPhotoWithAudio" : strongSelf.needShowPhotoWithAudioEdit
//                        ]
//
...
//            }
        ...
        self.extensionController.perform(extensionType: isInlineCamera ? .inlineCamera : .camera, with: [
            ...
            "isPhotoWithAudio" : needShowPhotoWithAudioEdit
            ])
    }
    
    private func showVideoCapture() {
        let didFinishCapturingVideo: (URL) -> Void = { [weak self] (videoURL: URL) in
            self?.presentingViewController?.dismiss(animated: true, completion: {
                self?.didSelectVideo?(videoURL)
            })
        }
        
        extensionController.perform(extensionType: .videoCapture, with: [
    ...
    @objc open func canSelect(asset: DKAsset, showAlert: Bool = true) -> Bool {
        ...
        if showAlert && assetType == .allVideos {
            if asset.duration > 301 {
                showVideoAlert("视频已超过5分钟时长\n不支持上传")
                return false
            }
            // 文件大小需要异步获取，待定
//            else if asset.fileSize > 100000 {
//                showVideoAlert("视频大小已超4G\n请到pc端上传")
//                return false
//            }
        }
    ...
    // MARK: - Photo with audio editor
    
    /// 需要显示图片+语音编辑器
    @objc public var needShowPhotoWithAudioEdit: Bool = false
        
    open func showPhotoWithAudioEditor() {
        let pwa = DKPhotoWithAudioEditor(assets: selectedAssets)
        pwa.ipc = self
        pwa.view.alpha = 0
        view.addSubview(pwa.view)
        addChild(pwa)
        pwa.view.snp.makeConstraints({ $0.edges.equalToSuperview() })
        var duration: TimeInterval = 0.25
        // 已显示DKPhotoGalleryController
