...
public extension DKAsset {
    ...
    @objc func fetchImage(with size: CGSize,
        ...
        if let lsize = lastSize, lsize == size, let cache = lastCacheImage {
            completeBlock(cache, nil)
        } else if self.originalAsset != nil {
            let fetchBlock: (_ image: UIImage?, _ info: [AnyHashable: Any]?) -> Void = { [weak self] image, info in
                self?.lastSize = size
                self?.lastCacheImage = image
