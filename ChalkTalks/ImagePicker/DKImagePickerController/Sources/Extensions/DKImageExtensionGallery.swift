...
open class DKPhotoGalleryController: DKPhotoGallery {
    ...
    open override func viewDidLoad() {
        ...
        galleryFooterView.snp.makeConstraints { (make) in
            ...
            make.height.equalTo(68 + DKUtils.safaAreaBottom)
        }
...
fileprivate class DKGalleryFooterView: UIView {
    ...
    func setup() {
        ...
        backBtn.snp.makeConstraints { (make) in
            ...
            make.centerY.equalToSuperview().offset(-DKUtils.safaAreaBottom/2)
    ...
    @objc func doneAction(_ btn: UIButton) {
        if let ext = gallery?.galleryDelegate as? DKImageExtensionGallery, let picker = ext.context.imagePickerController {
            if picker.selectedAssets.isEmpty {
                gallery?.dismissGallery()
                return
            }
            if !picker.showPhotoEditWhenDone {
                gallery?.transitionController = nil
            }
            picker.finish {
